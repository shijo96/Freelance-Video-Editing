...
if(isset($_POST['send'])){
    extract($_POST);
    $q="insert into chat (sender_id,receiver_id,message) values('$lid','{$_GET['elid']}','$reply')";
    insert($q);
}
?>
<div class="row">
    <div style="z-index: 1000;background-color: #fff"  align="center" >
        ...
        <div class="col-md-4" style="" >
            <h2 class="wow fadeInUp" style="font-family:monotype corsiva;"> <u>Chats with Editor </u></h2>
            ...
            <?php 
             $q="SELECT DISTINCT sender_id,CONCAT(fname,' ',lname) AS editor_name FROM `chat` INNER JOIN `editor` ON `editor`.`login_id`=`chat`.`receiver_id` WHERE receiver_id='{$_GET['elid']}' ";
            $row=select($q);
        ...
        <div class="col-md-6" style="margin-top: 50px;background-color: #fff;z-index: 1000">
                <div style="margin-right: 50px;align-content: left;width: 90%;padding-right: 20px" >
                    <h3 style="border-bottom:1px solid #fff;border-top:0px;font-weight:bold;font-size:30px;background-color: #bbb;padding-bottom: 5px;font-color:white" >
                       Chat with <?php if(sizeof($row)>0){ echo $row[0]['editor_name']; } ?></h3>
                    ...
                    <?php 
                        ...
                        $q="SELECT * FROM chat  WHERE sender_id='{$_GET['elid']}' or receiver_id='{$_GET['elid']}' ";
                        $res=select($q);
                        $i=1;
                    ?>
                    <?php foreach ($res as $row) :  ?>    
                        <tr style="">
                            ...
                            <?php if ($row['sender_id']==$_GET['elid']): ?>  
